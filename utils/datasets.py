...
class load_images_and_labels():
	def __init__(self, path, batch_size=1, img_size=416, augment=False):
		self.path = path
		image_folder = os.path.join(path, 'images')  # Path to images folder
		label_folder = os.path.join(path, 'labels')
		self.img_files = [os.path.join(image_folder, filename) for filename in os.listdir(image_folder) if filename.endswith(('.png', '.jpg'))]
		self.label_files=[os.path.join(label_folder, label_name ) for label_name in os.listdir(label_folder) ]
